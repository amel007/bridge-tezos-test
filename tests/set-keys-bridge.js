...
async function main(client) {
  ...
  const keysForBridge = [
    '0x'+tokeRootKeys.keys.public,
    '0x'+proxyKeys.keys.public,
    '0x'+tezosEventConfigurationKeys.keys.public
  ];

  console.log(keysForBridge);
  ...
  console.log("set keys for bridge:", bridgeAddr);


  const { body } = (await client.abi.encode_message_body({
    abi: bridgeAcc.abi,
    call_set: {
      function_name: "setKeys",
      input: {
        keys: keysForBridge,
      },
    },
  ...
  console.log("keys for bridge:", bridgeAddr, response.decoded.output);
}
