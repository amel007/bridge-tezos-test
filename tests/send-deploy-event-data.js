...
async function main(client) {
  ...
  const { body } = (await client.abi.encode_message_body({
    abi: tezosEventCOnfigurationAcc.abi,
    call_set: {
  ...
  response = await ownerNTDAcc.run("sendTransaction", {
    dest: tezosEventConfigurationAddr,
    value: 1600000000,
    bounce: true,
    flags: 3,
    payload: body,
  ...
  console.log("deploy event for configuration:", tezosEventConfigurationAddr, response.decoded.output);
}
